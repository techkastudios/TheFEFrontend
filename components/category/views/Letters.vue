...
export default defineComponent({
  ...
  setup() {
    ...
    const nextPageUrl = ref(`api/en/category/views/letters`)
      ...
      try {
        const response: CategoryViewsChild =
          await $axios.$get<CategoryViewsChild>(
            `api/${lang}/category/views/letters`
          )

        nextPageUrl.value = `api/${lang}/category/views/sub-more/letters/${response.id}`
...
<template>
  <div id="letters" ref="target" class="py-4">
    <v-container>

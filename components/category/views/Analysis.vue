...
export default defineComponent({
  ...
  setup() {
    ...
    const nextPageUrl = ref(`api/en/category/views/analysis`)
      ...
      try {
        const response: CategoryViewsChild =
          await $axios.$get<CategoryViewsChild>(
            `api/${lang}/category/views/analysis`
          )

        nextPageUrl.value = `api/en/category/views/sub-more/analysis/${response.id}`
        const items: Posts[] = [response.featured, ...response.posts]
        posts1.value = items.slice(0, 1)
        posts2.value = items.slice(1, 3)
      } finally {
        loading.value = false
      }
    }
...
<template>
  <div id="analysis" ref="target" class="py-4">
    <v-container>
          ...
          <div class="col-span-full">
            <div class="grid grid-cols-1 xl:grid-cols-12 xl:grid-rows-2 gap-4">
              <article
                ...
                :key="post.id"
                class="col-auto xl:col-span-6 xl:row-span-2"
              >
                <nuxt-link :to="localePath(post.slug)">
                  <figure v-if="post.image" class="relative h-full">

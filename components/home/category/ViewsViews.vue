...
export default defineComponent({
  ...
  setup() {
    ...
    const fetch = async () => {
      loading.value = true
      try {
        const category: CategoryViews = await $axios.$get<CategoryViews>(
          `api/${lang}/category/views/home`
        )
...
<template>
  <div id="home-category-views" ref="target" class="bg-bg2-light dark:bg-bg2-dark py-8">
    <v-container>
      <!-- heading -->
      <category-heading :name="name" :url="`/page${slug}`" />
